...
class Rocker:
    ...
    def __init__(self, ser: MultiSerial, time_delta: float = 0.3, strict: bool = True) -> None:
        self.ser = ser
    ...
    def callback(self, data: str):
        ...
        if match:
            ...
            i = 0
            for i, (action, t) in enumerate(self.actions):
                if t > ti:
                    break
            if i:
...
class RockerViewer:
    ...
    def __init__(self, rocker: Rocker, win: tk.Tk | None = None):
        self.rocker = rocker
        if win is None:
            win = tk.Tk()
    ...
    def mainloop(self):
        win = self.win
        label = tk.Label(win, text="", width=40, height=2)
        label.pack()
